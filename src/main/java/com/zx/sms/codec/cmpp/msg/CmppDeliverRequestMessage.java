	...
	public String toString() {
		return String
				.format("CmppDeliverRequestMessage [msgId=%s, destId=%s, serviceid=%s, tppid=%s, tpudhi=%s, msgfmt=%s, srcterminalId=%s, srcterminalType=%s, registeredDelivery=%s, msgLength=%s, msgContent=%s, reportRequestMessage=%s, linkid=%s, msgContentBytes=%s, isReport=%s, getPacketType()=%s, getTimestamp()=%s, getChannelIds()=%s, getChildChannelIds()=%s, getLifeTime()=%s, isTerminationLife()=%s, getResponse()=%s,  getHeader()=%s]",
						msgId, destId, serviceid, tppid, tpudhi, msgfmt, srcterminalId, srcterminalType, registeredDelivery, msgLength, msgContent,
						reportRequestMessage, linkid, Hex.encodeHexString(msgContentBytes), isReport, reserved, getPacketType(), getTimestamp(),
						getChannelIds(), getChildChannelIds(), getLifeTime(), isTerminationLife(), getResponse(), getHeader());
	}
