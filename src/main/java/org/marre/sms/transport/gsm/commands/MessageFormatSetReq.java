...
public class MessageFormatSetReq
{
    ...
    {
        log_.debug("Sending AT+CMGF command");
        comm.send("AT+CMGF=" + mode_ + "\r\n");
        log_.debug("Read response from AT+CMGF command. Expecting a OK");
        
        String rsp = comm.readLine();
        if (! rsp.startsWith("OK")) {
            throw new GsmException("AT+CMGF failed.", rsp);
        }
    }
}
