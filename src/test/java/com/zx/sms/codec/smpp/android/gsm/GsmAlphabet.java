...
public class GsmAlphabet {
    ...
    /**
     * char to GSM alphabet char
     * @param throwException If true, throws Exception on invalid char.
    ...
    gsmToChar(int gsmChar) {
        return (char)(gsmToChar.get(gsmChar) == null ?  ' ' :gsmToChar.get(gsmChar)) ;
    }
    ...
    gsmExtendedToChar(int gsmChar) {
        ...
        ret = gsmExtendedToChar.get(gsmChar);
    ...
    gsm8BitUnpackedToString(byte[] data, int offset, int length) {
        ...
        for (int i = offset ; i < offset + length ; i++) {
            ...
            if (c == 0xff) {
                ...
                if (prevWasEscape) {
                	Character cc = gsmExtendedToChar.get(c);
                	
                    ret.append((char)(cc == null?' ':cc));
                } else {
                	Character cc = gsmToChar.get(c);
                    ret.append((char)(cc == null?' ':cc));
                }
                prevWasEscape = false;
            }
     ...
     * @param throwsException If true, throws Exception if unencodable
     * char. Otherwise, counts invalid char as 1 septet
     */
    public static int
    countGsmSeptets(char c, boolean throwsException) throws Exception {
        if (charToGsm.get(c) != null) {
            return 1;
        }

        if (charToGsmExtended.get(c) != null) {
            return 2;
        }
    ...
    private static final Map<Character,Integer> charToGsm = new HashMap<Character,Integer>();
    private static final Map<Integer,Character> gsmToChar = new HashMap<Integer,Character>();
    private static final Map<Character,Integer> charToGsmExtended = new HashMap<Character,Integer>();
    private static final Map<Integer,Character> gsmExtendedToChar = new HashMap<Integer,Character>();
...
//        charToGsm.put('\u0147', i++);
        ...
        for(Entry<Character,Integer> entry: charToGsm.entrySet()) {
        	Character c = entry.getKey();
        	Integer v= entry.getValue();
        	 gsmToChar.put(v, c);
        }

        for(Entry<Character,Integer> entry: charToGsmExtended.entrySet()) {
        	Character c = entry.getKey();
        	Integer v= entry.getValue();
        	gsmExtendedToChar.put(v, c);
        }
